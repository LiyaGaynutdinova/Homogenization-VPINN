...
from PDE_losses import PDE_loss, get_areas, compute_bound
...
def train_primal(net, loaders, args, a_function, H, Lx):
    ...
    for epoch in range(args['epochs']):
        ...
        L = 0
        net.train()
        l, q = PDE_loss(x, net, a_function, H)
        ...
        losses_val.append(bound[0].item())

        print(f'Epoch: {epoch} mean train loss: {L : .8e}, bound: {bound[0].item() : .8e}')
        if (epoch+1)%1000 == 0:
